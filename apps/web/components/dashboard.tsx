    ...
    <LoadingState loading={loading} error={error}>
      {data && <>
        <div className="grid stats">
          ...
          <Stat tone="orange" label="结果未知" value={String(data.unknownPayments)} note="需要主动查单" />
          ...
          <Stat tone="red" label="需要人工处理" value={String(data.exhaustedRecoveries)} note="自动查单已达到上限" />
          <Stat tone="orange" label="未匹配账单" value={String(data.unmatchedReceipts)} note="等待订单或退款单出现" />
          <Stat tone="red" label="对账差错" value={String(data.mismatchedReceipts)} note="金额或流水号冲突" />
          <Stat tone="red" label="支付异常" value={String(data.openPaymentExceptions)} note="待核实、退款或人工关闭" />
          <Stat tone="orange" label="过期关闭异常" value={String(data.expirationFailures)} note="Worker 将自动退避重试" />
          <Stat tone="red" label="今日管理失败" value={String(data.failedAdminActionsToday)} note="可在操作审计中查看" />
        </div>
