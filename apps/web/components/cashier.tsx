...
export function Cashier({ paymentNo }: { paymentNo: string }) {
  ...
  return <div className="cashier-shell">
    <header className="cashier-brand">
      <div className="brand-badge">T</div>
      <div className="cashier-brand-copy"><strong>TUOXIN PAY</strong><span>拓昕支付 · 安全收银台</span></div>
    ...
    {data && <div className="cashier-layout">
      <section className="card cashier-main">
        <div className="cashier-main-head">
          <div><ChannelTag code={data.channel} /><h1>{data.subject}</h1></div>
          <Status value={data.status} />
        </div>

        <div className="cashier-amount-block">
          <span>本次应付</span>
          <strong>{money(data.amount)}</strong>
          <small>{data.currency || "CNY"} · 金额以当前收银台显示为准</small>
        </div>
        ...
        {canPay && !paid && <div className="cashier-duo">
          <div className="cashier-duo-item"><span>订单状态</span><strong className="cashier-live"><i aria-hidden="true" />等待付款</strong></div>
        ...
        <div className="cashier-foot">
          <span>支付单 <b className="mono">{data.paymentNo}</b></span>
          <span className="muted">{statusText(data.status)} · 系统自动确认</span>
        </div>
      </section>
    </div>}
  </div>;
