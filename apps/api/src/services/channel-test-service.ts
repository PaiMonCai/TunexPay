...
import { closePayment, createPayment } from "./payment-service.js";

export async function createChannelTest(id: string, revision: number, amountCents: number) {
  ...
  try {
    ...
    if (row.testPaymentNo && row.testRevision === revision) {
      ...
      if (previous && previous.status === "PROCESSING" && previous.amount === amountCents && previous.clientPayload && previous.order.expiresAt && previous.order.expiresAt > new Date()) {
        return { paymentNo: previous.paymentNo, cashierUrl: `${config().WEB_PUBLIC_URL}/cashier/${previous.paymentNo}` };
      }
      // A finished or stale test must not block the next run; close any open attempt.
      if (previous && ["CREATED", "PROCESSING", "UNKNOWN"].includes(previous.status)) {
        await closePayment(null, previous.paymentNo, "CHANNEL_TEST").catch(() => undefined);
      }
    }
    ...
    const { order } = await createOrder(app, { externalOrderNo: `check-${randomUUID()}`, amount: amountCents, currency: "CNY", subject: `通道验收 · ${row.name}`, expiresInSeconds: 300, metadata: { channelId: id, channelRevision: revision, diagnostic: true } }, undefined);
