...
import { assertPaymentTransition } from "../lib/state-machine.js";
...
export async function createPayment(application: Application, orderNo: string, input: CreatePaymentInput, idempotencyKey?: string) {
  ...
  const dispatch = await db.$transaction(async (tx) => {
    ...
    await tx.paymentEvent.create({ data: {
      ...
      type: "PAYMENT_CREATED", source: "API", payload: { channel, attemptNo: created.attemptNo },
    } });
    const processing = await tx.payment.update({ where: { id: created.id }, data: { status: "PROCESSING" } });
    await tx.paymentEvent.create({ data: {
      aggregateType: "PAYMENT", aggregateId: created.paymentNo, orderId: order.id, paymentId: created.id,
...
async function updatePaymentObservation(payment: Payment, status: PaymentStatus, data: Record<string, unknown>): Promise<Payment> {
  assertPaymentTransition(payment.status, status);
  return db.$transaction(async (tx) => {
    const updated = await tx.payment.update({ where: { id: payment.id }, data: { status, ...data } });
    await tx.paymentEvent.create({ data: {
      aggregateType: "PAYMENT", aggregateId: payment.paymentNo, orderId: payment.orderId, paymentId: payment.id,
      type: `PAYMENT_${status}`, source: "CHANNEL", payload: {
...
export async function markPaymentSucceeded(result: ChannelWebhookResult, source: string): Promise<Payment> {
  return db.$transaction(async (tx) => {
    ...
    if (isLateDuplicate) return payment;
    const updatedOrder = await tx.order.update({ where: { id: order.id }, data: { status: "SUCCESS", paidAt, winningPaymentId: payment.id } });
    await tx.paymentEvent.create({ data: {
...
export async function queryPayment(applicationId: string | null, paymentNo: string) {
  ...
  if (result.status === "SUCCESS") {
    const succeeded = await markPaymentSucceeded({
      ...
      raw: result.raw as Record<string, string>,
    }, "QUERY");
    return presentPayment(succeeded);
  }
  if (payment.status !== result.status && payment.status !== "SUCCESS") {
    return presentPayment(await updatePaymentObservation(payment, result.status, { rawResponse: result.raw as Prisma.InputJsonValue }));
  }
...
export async function closePayment(applicationId: string, paymentNo: string) {
  const payment = await db.payment.findFirst({ where: { paymentNo, order: { applicationId } } });
  if (!payment) throw new AppError("PAYMENT_NOT_FOUND", "支付单不存在", 404);
  ...
  const updated = await db.$transaction(async (tx) => {
    ...
    const closed = await tx.payment.update({ where: { id: current.id }, data: { status: "CLOSED", rawResponse: result.raw as Prisma.InputJsonValue } });
    await tx.paymentEvent.create({ data: {
      aggregateType: "PAYMENT", aggregateId: current.paymentNo, orderId: current.orderId, paymentId: current.id,
      type: "PAYMENT_CLOSED", source: "API", payload: {},
    } });
    return closed;
  });
  return presentPayment(updated);
