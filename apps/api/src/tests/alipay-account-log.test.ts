...
const payment = { income: "19.99", outcome: "0.00", alipay_order_no: "202609160001", account_log_id: "log-1", trans_dt: "2026-09-16 12:00:00", trans_memo: "会员 TXA1B2C3D4E5" };
...
describe("Alipay account ledger contract", () => {
  ...
  it("ignores outgoing, unidentified income, and explicit refund entries", () => {
    expect(paymentFlowFromAccountLog({ ...payment, income: "0.00", outcome: "19.99" })).toBeNull();
    expect(paymentFlowFromAccountLog({ ...payment, outcome: "0.01" })).toBeNull();
  ...
  it("fails closed on malformed amounts and missing time", () => {
    expect(() => paymentFlowFromAccountLog({ ...payment, income: "19.999" })).toThrow();
    expect(() => paymentFlowFromAccountLog({ ...payment, outcome: undefined })).toThrow();
  ...
  it("requires total_size and consistent pagination instead of treating schema errors as empty bills", () => {
    expect(accountLogPage({ total_size: 101, account_log_list: Array(100).fill(payment) }, 1, 100).complete).toBe(false);
    expect(accountLogPage({ total_size: "101", account_log_list: [payment] }, 2, 100).complete).toBe(true);
    expect(accountLogPage({ total_size: 0, account_log_list: [] }, 1, 100).complete).toBe(true);
    expect(() => accountLogPage({}, 1, 100)).toThrow();
    expect(() => accountLogPage({ total_size: 101, account_log_list: [] }, 1, 100)).toThrow();
    expect(() => accountLogPage({ total_size: 101, account_log_list: [payment] }, 1, 100)).toThrow();
  });
