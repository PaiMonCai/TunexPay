...
async function findApplication(params: Record<string, string>): Promise<Application> {
  ...
  // 归档应用的 ePay 凭证同样立即失效，接口一律按「商户不存在」处理。
  if (!application || application.archivedAt || application.status !== "ACTIVE") throw new AppError("EPAY_AUTH_FAILED", "商户不存在或已禁用", 401);
  return application;
}
  ...
  try {
    ...
    if (act === "query") {
      ...
      const [orders, orderToday, orderLastday] = await Promise.all([
        db.order.count({ where: { applicationId: application.id, deletedAt: null } }),
        db.order.count({ where: { applicationId: application.id, deletedAt: null, createdAt: { gte: start } } }),
        db.order.count({ where: { applicationId: application.id, deletedAt: null, createdAt: { gte: yesterday, lt: start } } }),
      ]);
    ...
    if (act === "orders") {
      ...
      const orders = await db.order.findMany({ where: { applicationId: application.id, deletedAt: null }, include: { payments: { orderBy: { attemptNo: "desc" }, take: 1 } }, orderBy: { createdAt: "desc" }, skip: (page - 1) * limit, take: limit });
